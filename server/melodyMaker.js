...
function createScale(tonic, scale) {
  ...
  validNotes.forEach((note) => {
    if (
      Object.keys(noteFrequencyMap).indexOf(note) <
      Object.keys(noteFrequencyMap).indexOf(lastNote)
    ) {
      octave++;
    }
    // eslint-disable-next-line functional/immutable-data
...
function createMelody() {
  ...
  var notesArray = [];
  const noteLengths = [0.25, 0.5];
  var time = 0;
  // eslint-disable-next-line functional/no-loop-statement
  while (time < 10) {
    var thisNoteLength =
      noteLengths[Math.floor(Math.random() * noteLengths.length)];
    // eslint-disable-next-line functional/immutable-data
    notesArray.push([
      pickValidRandomNote(validNotes),
      time,
      time + thisNoteLength,
    ]);
    time += thisNoteLength;
...
function getValidNotes(tonic, scaleIntervals) {
  ...
  return scaleIntervalArray.map(
    (interval) => orderedNotes[IVLS.indexOf(interval)]
  );
}
...
const SCALES = [
  ...
  ['1P 3m 3M 5d 7m', 'super locrian pentatonic'],
